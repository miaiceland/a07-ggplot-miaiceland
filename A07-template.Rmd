...
ggplot(data = Trips) + 
  geom_boxplot( aes(x = client, y = dur <- as.numeric(edate - sdate)/60)) +
  ylim(0,60) +
  ylab("dur/60")
...
### Your Code Here
ggplot(data = Trips) + 
  geom_density( aes(x = lubridate::yday(sdate)), fill = "gray") +
...
The data was collected during the winter time of the year; only days 270 through 365 were recorded. Bikes were most used during the month of October or between days 295 and 300.
...
### Your Code Here
ggplot(data = Trips) + 
  geom_density( aes(x = lubridate::wday(sdate)), fill = "gray") +
...
The bikes are used frequently everyday of the week. The big dips indicate nighttime when bikes are less frequently used because most people are sleeping. Day 5 and 6 are when the bikes are used the most and day 7 and 1 is when the bikes are used the least.
...
### Your Code Here
ggplot(data = Trips) + 
  geom_density( aes(x = lubridate::hour(sdate)), fill = "gray") +
...
Again, the hours 0 through 5 is when the bikes are used the least because 12am to 5am is nighttime (when people are sleeping). The bikes are used the most during 17:30 (5:30pm) and a smaller spike at 8am. 
...
### Your Code Here
ggplot(data = Trips) + 
  geom_density( aes(x = lubridate::minute(sdate)), fill = "gray") +
...
The minute of the hour graph does not tell me much. There are slight ripples in the graph but for the most part, the bikes are used throughout the hour. 
...
ggplot(data = Trips) + 
  geom_histogram( aes(x = lubridate::hour(sdate), color = client)) + 
  xlab("Hour of the Day") +
  ylab("Count") +
  facet_wrap(~ lubridate::wday(sdate))
  

```

I have found that the majority of people that use the rental bikes are registered than guests. Also, each day of the week, there are about 4 waves in which the bikes are used at a high frequency. The busiest day to rent a bike is day 2,5 and 6 of the week. 
...
Trip_Distance <-
  Trips %>%
  inner_join(PairDistances, join_by(sstation, estation))

ggplot(data = Trip_Distance) + 
...
Simple and Simple2 had different variable names for latitude and longitude even though they are the same values because of the haversine() function. The haversine() function finds the distance between 2 locations using their latitude and longitude. Because we want to find the distance between every location (including the distance from itself to itself) it would be the least confusing to the computer if we labeled the locations as unique for each table. This way we can merge the tables and still have the computer see the lat and long for each table as different/unique values. 
